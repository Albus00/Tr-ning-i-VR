...
public class EnemySpawner : MonoBehaviour // spawns enemy with "spawnInterval" seconds inbetween
{
  ...
  private void Update()
  {
    ...
    {
      // Generate space in front of player (now back though??) based on the selected difficulty
      Vector3 enemySpawn = transform.position +
        player.transform.forward * (5 / (float)difficulty) +
        player.transform.right * (Random.Range(-5, 5) / (float)difficulty);
      enemySpawn.y = 1f;                                    // Make sure enemy spawns above ground
      Instantiate(enemy, enemySpawn, transform.rotation);   // Spawns enemy
